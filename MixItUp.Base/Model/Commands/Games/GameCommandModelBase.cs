...
namespace MixItUp.Base.Model.Commands.Games
{
    ...
    public abstract class GameCommandModelBase : ChatCommandModel
    {
        ...
        protected async Task RefundCooldown(CommandParametersModel parameters)
        {
            foreach (CooldownRequirementModel cooldown in this.Requirements.Cooldowns)
            {
                await cooldown.Refund(parameters);
            }
        }
        ...
        protected async Task PerformCooldown(CommandParametersModel parameters)
        {
            foreach (CooldownRequirementModel cooldown in this.Requirements.Cooldowns)
            {
                await cooldown.Perform(parameters);
            }
        }
