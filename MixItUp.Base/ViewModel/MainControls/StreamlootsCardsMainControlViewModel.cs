    ...
    public class StreamlootsCardsMainControlViewModel : GroupedCommandsMainControlViewModelBase
    {
            ...
            : base(windowViewModel)
        {
            ...
            this.StreamlootsManageCollectionCommand = this.CreateCommand((parameters) =>
            {
                ...
                {
                    ServiceManager.Get<IProcessService>().LaunchLink($"https://www.streamloots.com/{ServiceManager.Get<VeloraSession>().StreamerUsername}/manage/cards");
                }
                else if (ServiceManager.Get<VPZoneSession>().IsConnected)
                {
                    ServiceManager.Get<IProcessService>().LaunchLink($"https://www.streamloots.com/{ServiceManager.Get<VPZoneSession>().StreamerUsername}/manage/cards");
                }
            });
        }
