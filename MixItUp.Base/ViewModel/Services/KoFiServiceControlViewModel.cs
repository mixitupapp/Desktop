...
{
    public class KoFiServiceControlViewModel : ServiceControlViewModelBase
    {
        ...
        public override string WikiPageName { get { return "ko-fi"; } }

        public KoFiServiceControlViewModel()
            : base(Resources.KoFi)
        {
            this.ConnectCommand = this.CreateCommand(async () =>
            {
                Webhook webhook = await ServiceManager.Get<MixItUpService>().CreateWebhook(WebhookServices.Kofi);
                if (webhook != null)
                {
        ...
        protected override async Task OnOpenInternal()
        {
            ...
            try
            {
                ...
                Webhook webhook = response?.Webhooks?.FirstOrDefault(w => string.Equals(w.Service, WebhookServices.Kofi, StringComparison.OrdinalIgnoreCase));
                if (webhook != null)
                {
                    this.SetConnectedWebhook(webhook);
                }
