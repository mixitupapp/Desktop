...
{
    public class ThroneServiceControlViewModel : ServiceControlViewModelBase
    {
        ...
        public override string WikiPageName { get { return "throne"; } }

        public ThroneServiceControlViewModel()
            : base(Resources.Throne)
        {
            this.ConnectCommand = this.CreateCommand(async () =>
            {
                Webhook webhook = await ServiceManager.Get<MixItUpService>().CreateWebhook(WebhookServices.Throne);
                if (webhook != null)
                {
