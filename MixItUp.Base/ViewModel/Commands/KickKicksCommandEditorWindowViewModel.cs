...
{
    public class KickKicksCommandEditorWindowViewModel : CommandEditorWindowViewModelBase
    {
        public bool IsRange
        {
        ...
        public KickKicksCommandEditorWindowViewModel(KickKicksCommandModel existingCommand)
        ...
        public KickKicksCommandEditorWindowViewModel() : base(CommandTypeEnum.KickKicks) { }
        ...
        public override Task<Result> Validate()
        {
            ...
            foreach (KickKicksCommandModel command in ServiceManager.Get<CommandService>().KickKicksCommands)
            {
                    ...
                    (!this.IsRange && !command.IsRange && command.StartingAmount == this.StartingAmount))
                {
                    ...
                    {
                        return Task.FromResult(new Result(Resources.KickKicksCommandsAlreadyExistsDuplicateAmount));
                    }
                }
            }
        ...
        public override Task<CommandModelBase> CreateNewCommand()
        {
            if (this.IsRange)
            {
                return Task.FromResult<CommandModelBase>(new KickKicksCommandModel(this.Name, this.StartingAmount, this.EndingAmount));
            }
            else
            {
                return Task.FromResult<CommandModelBase>(new KickKicksCommandModel(this.Name, this.StartingAmount, this.StartingAmount));
            }
        }
        ...
        public override async Task UpdateExistingCommand(CommandModelBase command)
        {
            ...
            ((KickKicksCommandModel)command).StartingAmount = this.StartingAmount;
            ((KickKicksCommandModel)command).EndingAmount = (this.IsRange) ? this.EndingAmount : this.StartingAmount;
        }

        public override Task SaveCommandToSettings(CommandModelBase command)
        {
            ServiceManager.Get<CommandService>().KickKicksCommands.Remove((KickKicksCommandModel)this.existingCommand);
            ServiceManager.Get<CommandService>().KickKicksCommands.Add((KickKicksCommandModel)command);
